...
const CrimeMap = () => {
  ...
  const initializeMap = async () => {
    ...
    const geolocateControl = new (window.L.Control.extend({
      onAdd: function () {
          ...
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition((position) => {
              ...
              map.current.eachLayer(layer => {
                if (layer.options && (layer.options as any).isUserLocation) map.current.removeLayer(layer);
              });
              const marker = window.L.marker([latitude, longitude])
                .addTo(map.current)
                .bindPopup('<b>You are here</b>')
                .openPopup();
              (marker as any).options.isUserLocation = true;
            }, () => {
        ...
        <div style="font-family: system-ui, sans-serif; min-width: 200px;">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
            ...
            <span style="background-color: ${(() => {
              const iconHtml = createCustomIcon(crime.priority).options.html;
              const match = typeof iconHtml === 'string' ? iconHtml.match(/background-color: ([^;]+);/) : null;
              return match ? match[1] : '#6b7280';
            })()}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 10px; text-transform: uppercase; font-weight: bold;">
              ${crime.priority}
            </span>
          </div>
